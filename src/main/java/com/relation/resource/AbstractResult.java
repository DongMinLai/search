...
public abstract class AbstractResult<T, R> implements Resource<T, R> , Serializable {
    ...
    @Override
    public abstract void modifyItem(ModifyItem modifyItem);
    ...
    protected LinkedHashMap<String, Object> _initMap(T data){

        try {
            List<Method> methods = getMethods(data);
            LinkedHashMap<String, Object> map = new LinkedHashMap<>(methods.size());
     ...
     * 去除非配置属性
     */
    protected void formatByListConfig(LinkedHashMap<String, Object>map)
    {
        if(listConfig != null){
            Set<String> shows = listConfig.getListConfig()
                    .getFields()
                    .stream()
                    .map(FieldVo::getCode)
